...
def get_val(root):
    ...
    """
    with open('train_val_test_json/part_B_val.json', 'r') as f:
    ...
    with open('train_val_test_json/B_val.json', 'w') as f:
        json.dump(new_val, f)
...
def get_train(root):
    ...
    with open('train_val_test_json/B_train.json', 'w') as f:
...
def get_test(root):
    ...
    filenames = os.listdir(path)
    pathname = [os.path.join(path, filename) for filename in filenames]
    with open('train_val_test_json/B_test.json', 'w') as f:
        json.dump(pathname, f)
